...
12 §4-2でメール配信を初期の主力チャネルと決めたため、メルマガ登録を独立したCVとして扱う。

Report DLとの違い：

| | Report Download | Newsletter |
| --- | --- | --- |
| 取得項目 | 会社名・URL・氏名・役職・業界・Email | **Emailのみ**（任意で会社名） |
| 心理的ハードル | 中 | 低 |
| リードの質 | 高い（企業が特定できる） | 低い（企業が不明なことがある） |
| 役割 | 取材候補・相談候補の抽出 | 関係の継続。将来のReport DLへの導線 |

Newsletterで入力項目を増やさない。会社URLを必須にすると、Report DLと同じハードルになり、別CVとして用意する意味がなくなる。

Company が特定できない Person が発生するが、これは許容する。後から Report DL や 相談で会社情報が入った時点で、Email をキーに Person を紐づけ、Company を後付けする。

## 配信同意との関係

Newsletterは登録行為そのものが配信同意になる。

一方 Report Download では、B-4（11）で決めたとおり配信同意のチェックボックスを別途置く。DLと配信を同一の同意で処理しない。
...
上記4つと並列ではなく、**全ページに常設する低ハードルのCV**として扱う。

初期は自然検索流入が少なく、Report DLに至る母数自体が小さい。Newsletterは、まだ関係のない読者との最初の接点を作る役割を持つ。
