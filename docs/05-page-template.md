...
# 16-2. ブランド4視点の置き場所
00 §2-2の「採る・活かす・減らす・強くする」は、01 §4でSEO分類と分離すると決めた。分離した結果、この4視点の置き場所が必要になる。

## Phase 1

TOPの1セクション（05 §14）とAbout内のセクションに置く。独立ページは作らない。

## Phase 2以降

コンテンツが蓄積したら、独立ページ化を検討する。

```
/concept/hire/          採る
/concept/enable/        活かす
/concept/reduce/        減らす
/concept/strengthen/    強くする
```

各ページには、その視点についての思想文と、該当する読者の悩み、そこから関連コンテンツへの導線を置く。

**テーマハブ（/recruiting/ 等）との違い：**

| | Theme Hub | Concept ページ |
| --- | --- | --- |
| 目的 | 検索意図の獲得 | ブランド思想の説明と回遊 |
| 入口 | 検索 | TOP・About・サイト内回遊 |
| 文体 | 課題解決・体系的 | 思想・問いかけ |
| index | する | する（独自本文があるため） |

「採る」と「採用・人材確保」は概念として重なるが、ページの目的が異なる。検索意図が重複しないよう、Conceptページは検索キーワードを狙わない。

Phase 2で実装する場合、02 §18のSEOランディングページ追加基準を満たすか確認する。満たさないなら作らない。
...
# 65-2. メルマガ登録CTA
12 §4-2でメール配信を初期の主力チャネルと決めたため、メルマガ登録をReport DLに次ぐ常設CTAとして扱う。

配置：

- Article本文末尾（関連Reportの次）
- Theme Hub / Industry Hub の下部
- Report一覧・Interview一覧の下部
- フッター

Report DLと違い、**登録の心理的ハードルが低い**点が重要。関連Reportがまだ存在しない記事でも、行き止まりにせず次の接点を作れる。

表現は営業的にしない。「人手不足研究所から、調査結果と取材記事をお送りします」程度にとどめる。頻度を明示する（例：月2回程度）。

Report DL CTAと同一画面に2つ並べない。どちらか一方にする。
...
「記事を探す」系のUIでは、保有タグを機械的に並べない。**編集がキュレーションした少数のキーワードだけを出す。**
...
タグIDそのもの（`labor-shortage` / `excel`）を表示名に使わず、**読者の悩みの言葉**でラベルを付ける。00 §4で整理した顧客課題がそのまま使える。

これにより、01 §16「すべてのタグページを生成しない」を守りながら、探索UIを成立させられる。表示するキーワードはTag Masterに `featured: true` を持たせて制御する。

# 81-2. 連載・シリーズの扱い
Reportを中心とした記事群（02 §31のコンテンツクラスター）は、読者から見ると連載に近い。これをUI上でも見せる。

ただし**「連載」として定期公開を約束しない**（12 §7-5）。次回予告や公開スケジュールを出さない。

実装方針：

- Reportページに「このレポートから派生した記事」セクションを持つ
- 各Articleに「このレポートの関連記事」として同クラスターの記事を並べる
- 独立した連載一覧ページはPhase 1では作らない

データ上は03の `relatedContent` で表現できるため、新しいモデルは追加しない。Phase 2以降でクラスターが複数できた段階で、独立したシリーズページを検討する。

# 82. シェア機能
Article / Report / InterviewにSNS共有機能を設置可能とする。

**Interviewについては初期から実装する。** 12 §4-4で「取材企業が自社サイト・SNSで紹介しやすい状態を作る」と決めたため、共有導線がないと機会を落とす。

対応先：X / Facebook / LINE / LinkedIn。読者層が経営者・管理部門であることを踏まえ、LINEを外さない。

Article / Reportは初期実装の優先度を下げてよい。
...
空でなくても、件数が少なすぎるセクションは出さない。コンテンツが少ないことが露呈し、10 §6の「空のメディアに見える」状態になる。
...
| TOP 最新記事 | 3件 |
| TOP インタビュー | 1件 |
| TOP 業界から探す | 公開済み業界ハブが1つ以上 |
| Theme Hub 業界別導線 | 該当業界の記事が2件以上 |
| Theme Hub Interview | 1件 |
| 関連コンテンツ | 2件 |

条件を満たさない場合はセクションごと非表示にする。件数を満たすまでは、Hubの固定編集コンテンツ（テーマ解説・主要データ・論点整理）だけで成立する構成にしておく。

つまり**Hubの上部（編集領域）を厚く作り、下部（自動フィード領域）が空でも破綻しない**設計にする。
