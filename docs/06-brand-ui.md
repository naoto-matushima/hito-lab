...
# 101-2. 初期の情報密度
参考にする既存メディアは、記事数が数百〜数千ある前提でレイアウトが組まれている。カードが十数個並ぶグリッド、ランキング枠、タグ一覧、複数のサイドバーモジュール。

**これを記事3本の状態で模倣すると、スカスカに見える。**

初期は密度を意図的に落とす。

| 要素 | 参考メディアの典型 | 人手不足研究所の初期 |
| --- | --- | --- |
| TOPのカード枚数 | 20〜40 | 8〜12 |
| 1行あたりのカード数（PC） | 3〜4 | 2〜3 |
| サイドバー | あり（ランキング・タグ・広告） | なし |
| ランキング | あり | 作らない |
| タグ表示 | 記事あたり5個前後 | 2〜3個 |
| セクション数（TOP） | 10前後 | 6〜8 |

カードを減らした分の面積は、**1枚あたりを大きくする**ことで埋める。小さいカードを少数並べると余白が目立つが、大きいカードを少数置くと意図した構成に見える。

特に注目Reportは、TOPで画面幅の2/3以上を使ってよい。1本しかないことが弱点ではなく、「これが今の主要調査である」という提示になる。

コンテンツが増えたら密度を上げる。逆は難しい（スカスカな状態を見せた印象は残る）。

# 101-3. Eyecatchの初期運用
11 C-5への対応。取材写真が存在しない初期は、以下の優先順で作る。

1. **独自グラフ・データビジュアル** — Report・統計分析系のArticle
2. **タイポグラフィ主体のテンプレート** — 解説系Article
3. **図解イラスト** — 構造・フローを説明する記事
4. 実写 — Interview（取材時に撮影）

2のテンプレートは、Report Eyecatch（06 §53）と同じフォーマット思想で作る。

```
┌────────────────────────┐
│ 採用・人材確保          │  ← Theme ラベル（Sage）
│                        │
│ 建設業は本当に          │  ← タイトル（Zen Maru Gothic）
│ 人手不足なのか          │
│                        │
│ 人手不足研究所          │  ← ロゴ（小さく）
└────────────────────────┘
...
背景はテーマごとにWarm White / Pale Sage / Pale Terracottaを割り当てる。一覧に並んだときにテーマが色で判別でき、かつストックフォトの寄せ集めに見えない。

3のイラストは、記事の内容を理解した上で「どのキーワードを絵にするか」を決める必要がある。工数がかかるため、Report級のコンテンツに限定する。

写真がないことを隠すためにストックフォトを使わない。**データと図で構成した画面の方が、この媒体では信頼を作る**（06 §97）。
